<script>

import Gift from "./Gift.vue"
export default {

    components:{Gift},
    props: {
        number:{},
        hasGift: {type: Boolean}    // e uma propriedade
    } , 
    data: function(){
        // duas  funcoes aqui 
        //1 - para saber se a porta ta fechada ou não
        //2- se tem presente ou não tem presente
        return {
            open:false , // a porta não está  aberta e nem fechada
            selected:false
        }
    }
}
</script>

<template>
    <div class="door-area" >
        <div class="door-frame" :class="{selected: selected && !open}">
            <Gift v-if="open && hasGift"/>
        </div>
        <div 
        class="door "
        :class="{open}"
         @click="selected = !selected" >
            <div class="number" :class="{selected}"> {{ number }}</div>
        <div class="knob" :class="{selected}"
        @click.stop="open = true" > </div>
        </div>
    </div>
  
</template>

<style css >
:root{
    --door-border: 5px solid black;
    --selected-border:  5px solid yellow;
}

.door-area {
    position: relative;
    width: 200px;
    height: 310px;
    border-bottom: 10px solid #AAA;
    margin-bottom: 20px;
    font-size: 3rem;
    display: flex;
    justify-content: center;

}

.door-frame{
    position:absolute;
    height: 300px;
    width: 180px;
  

  
    display: flex;
    justify-content: center;
    align-items: flex-end;

}

.door{
    position: absolute;
    top: 5px;
    height: 295px;
    width: 180px;
    background: chocolate;

    border-top: var(--door-border);
    border-left: var(--door-border);
    border-right: var(--door-border);
 
    display: flex;
    flex-direction: column;
    align-items: center;
    padding: 20px;

}

.door .knob{
    height: 20px;
    width: 20px;
    border-radius: 10px;
    background-color: black;
    margin-top: 3.2rem;

    align-self: flex-end;

}



.door> .number.selected {
    color: yellow;
}


.door >.knob.selected{
    background-color: yellow;
}

.door.selected{
    background-color: yellow;
}

.door.open{
    background-color: black;
}


.door.open .knob{
    display: none;
}

.door.open .number{
    display: none;
}
</style>